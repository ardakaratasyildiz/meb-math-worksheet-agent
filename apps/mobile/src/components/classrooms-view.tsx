...
/**
 * Sınıf listesi + sınıf oluşturma — EKRANDAN BAĞIMSIZ gövde.
 *
 * İki yerde kullanılır: `app/classrooms.tsx` (itilen ekran, kendi başlığıyla) ve
 * öğretmen rolünde üçüncü SEKME (`(tabs)/progress.tsx`). Öğretmene kendi kişisel
 * gelişim panosunu göstermek anlamsızdı; sekme artık role göre bu gövdeyi render eder.
 */
export function ClassroomsView({ header }: { header?: React.ReactNode }) {
  ...
  const openClass = (id: string) => () =>
    router.push(`/classroom/${encodeURIComponent(id)}` as Href);
