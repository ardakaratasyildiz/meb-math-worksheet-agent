"""Aşama D yeni kazanımları için A/B testi.

Aşama D'de eklenen 16 yeni kazanımdan 6 tanesi seçilerek (sentetik+manuel) vs
(sentetik+manuel+textbook) üretimi karşılaştırılır.

    A) include_textbook=False — sadece sentetik corpus + manuel few-shot
    B) include_textbook=True  — A + ders kitabı chunk'ları

Metrikler:
    - Soru tipi dağılımı
    - Bağlam çeşitliliği (unique context token sayısı)
    - Token-level Jaccard benzerliği
    - Karakter uzunluk dağılımı

Çıktı: docs/AŞAMA_D_AB_REPORT.md + knowledge_base/processed/ab_test_aşama_d.json
"""
...
OUTPUT_JSON = ROOT / "knowledge_base" / "processed" / "ab_test_aşama_d.json"
OUTPUT_MD = ROOT / "docs" / "AŞAMA_D_AB_REPORT.md"

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", datefmt="%H:%M:%S")
logger = logging.getLogger("compare")


# Test kümesi — Aşama D yeni kazanımları (6 farklı kazanım, 3 farklı sınıf)
TEST_CASES = [
    {"grade": 5, "topic_id": "kesirler", "kazanim_kod": "M.5.2.5", "difficulty": Difficulty.ORTA, "n": 5},
    {"grade": 5, "topic_id": "veri_isleme", "kazanim_kod": "M.5.6.1", "difficulty": Difficulty.ORTA, "n": 5},
    {"grade": 6, "topic_id": "dogal_sayilar", "kazanim_kod": "M.6.1.5", "difficulty": Difficulty.ORTA, "n": 5},
    {"grade": 6, "topic_id": "veri_isleme", "kazanim_kod": "M.6.6.2", "difficulty": Difficulty.ORTA, "n": 5},
    {"grade": 6, "topic_id": "olasilik", "kazanim_kod": "M.6.7.1", "difficulty": Difficulty.ORTA, "n": 5},
    {"grade": 7, "topic_id": "olasilik", "kazanim_kod": "M.7.7.1", "difficulty": Difficulty.ORTA, "n": 5},
]
SEED = 1234
...
def _format_report(results: list[dict]) -> str:
    lines: list[str] = []
    lines.append("# Aşama D — Yeni Kazanımlar için A/B Raporu (Sentetik vs Sentetik+Textbook)\n")
