...
8. İstenen tip dağılımına TAM uy. Tip-spesifik formatlar:
   ...
   - ⛔ **`okuma_pasaji`, `kelime_bilgisi`, `dil_bilgisi`, `yazim_noktalama` DÖRT şıkkı MUTLAKA `options` alanına koy** (düz metin, harf öneki "A)" OLMADAN, doğru sırayla) — tıpkı `coktan_secmeli` gibi; `answer` = doğru şık harfi (A/B/C/D). "Aşağıdaki cümlelerin/ifadelerin hangisi…" deyip şıkları VERMEMEK KESİNLİKLE YASAK → soru cevaplanamaz, OTOMATİK ELENİR.
