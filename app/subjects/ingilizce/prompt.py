...
9. Follow the requested question-type distribution EXACTLY. Type-specific formats:
   - `coktan_secmeli` (PRIMARY type): The most common MEB pattern is **gap/blank completion** — a sentence or short dialogue ending with a blank (e.g. "I usually get up early because I - - - - to school.") + 4 options A) B) C) D) that complete it. Exactly one correct. Distractors from the SAME word field/category (all plausible words, one fits the context) OR common learner errors. `answer` = the correct option letter only ("A"/"B"/"C"/"D"). NOT/EXCEPT/CANNOT patterns are common in grades 7-8 ("Which of the following is NOT correct according to the text?").
