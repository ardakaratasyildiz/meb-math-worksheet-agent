...
9. Follow the requested question-type distribution EXACTLY. Type-specific formats:
   - `coktan_secmeli` (PRIMARY type): The most common MEB pattern is **gap/blank completion** — a sentence or short dialogue ending with a blank (e.g. "I usually get up early because I - - - - to school.") written in the `question` field. Put the **4 options in the `options` field as PLAIN TEXT (NO "A)" prefix), in order — EXACTLY 4, never a 5th**. Exactly one correct. Distractors from the SAME word field/category (all plausible words, one fits the context) OR common learner errors. `answer` = the correct option letter only ("A"/"B"/"C"/"D") — it maps to the position in `options`. NOT/EXCEPT/CANNOT patterns are common in grades 7-8.
